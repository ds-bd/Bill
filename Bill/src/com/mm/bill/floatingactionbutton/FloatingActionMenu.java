package com.mm.bill.floatingactionbutton;

import java.util.ArrayList;
import java.util.List;

import com.mm.bill.R;
...
public class FloatingActionMenu {
    ...
    public FloatingActionMenu(final View mainActionView, int startAngle, int endAngle, int radius,
            ...
            MenuStateChangeListener stateChangeListener, final boolean systemOverlay) {
            ...
            if (item.width == 0 || item.height == 0) {
                if (systemOverlay) {
                    throw new RuntimeException(
                            "Sub action views cannot be added without " + "definite width and height.");
                }
        ...
        if (systemOverlay) {
            orientationListener =
                    new OrientationEventListener(mainActionView.getContext(), SensorManager.SENSOR_DELAY_UI) {
    ...
    public void open(boolean animated) {
        ...
        if (animated && animationHandler != null) {
            ...
            for (int i = 0; i < subActionItems.size(); i++) {
                ...
                if (systemOverlay) {
                    params.setMargins(center.x - overlayParams.x - subActionItems.get(i).width / 2,
                            center.y - overlayParams.y - subActionItems.get(i).height / 2, 0, 0);
                } else {
            ...
            for (int i = 0; i < subActionItems.size(); i++) {
                ...
                final FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(subActionItems.get(i).width,
                        subActionItems.get(i).height, Gravity.TOP | Gravity.LEFT);
                if (systemOverlay) {
                    params.setMargins(subActionItems.get(i).x - overlayParams.x,
    ...
    public void attachOverlayContainer() {
        try {
            ...
            throw new SecurityException(
                    "Your application must have SYSTEM_ALERT_WINDOW " + "permission to create a system window.");
        }
    }
    ...
    public static class Builder {
        ...
        public Builder(Context context, boolean systemOverlay) {
            ...
            // Default settings
            radius = context.getResources().getDimensionPixelSize(R.dimen.red_action_menu_radius);
            startAngle = 180;
            endAngle = 270;
        ...
        public Builder addSubActionView(View subActionView) {
            if (systemOverlay) {
                throw new RuntimeException("Sub action views cannot be added without "
                        + "definite width and height. Please use " + "other methods named addSubActionView");
            }
        ...
        public FloatingActionMenu build() {
            return new FloatingActionMenu(actionView, startAngle, endAngle, radius, subActionItems, animationHandler,
                    animated, stateChangeListener, systemOverlay);
        }
    }
